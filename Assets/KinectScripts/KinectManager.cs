...
//	public bool NearMode = false;
	...
	private KinectCommon.NuiSkeletonFrame skeletonFrame;
	private KinectCommon.NuiTransformSmoothParameters smoothParameters;
	...
	private KinectCommon.NuiSkeletonBoneOrientation[] jointOrientations;
	...
	{
		int index = y * KinectCommon.Constants.DepthImageWidth + x;
	...
	{
		Vector3 vDepthPos = RemoteKinectWrapper.MapSkeletonPointToDepthPoint(posJoint);
	...
	public Vector2 GetColorMapPosForDepthPos(Vector2 posDepth)
	{
		...
		KinectCommon.NuiImageViewArea pcViewArea = new KinectCommon.NuiImageViewArea 
		{
            eDigitalZoom = 0,
            lCenterX = 0,
            lCenterY = 0
		...
		RemoteKinectWrapper.NuiImageGetColorPixelCoordinatesFromDepthPixelAtResolution(
			KinectCommon.Constants.ColorImageResolution,
			KinectCommon.Constants.DepthImageResolution,
			ref pcViewArea,
	...
	// returns the index of Player1, or 0 if no Player1 is detected
	...
	public bool IsPlayerCalibrated(ulong UserId)
	{
		if(UserId == Player1ID)
	...
	public Vector3 GetRawSkeletonJointPos(ulong UserId, int joint)
	{
		if(UserId == Player1ID)
	...
	{
		if(UserId == Player1ID && player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase])
			return ConvertMatrixToQuat(player1Ori, (int)KinectCommon.NuiSkeletonPositionIndex.SpineBase, flip);
		else if(UserId == Player2ID && player2JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase])
			return ConvertMatrixToQuat(player2Ori, (int)KinectCommon.NuiSkeletonPositionIndex.SpineBase, flip);
	...
	public Vector3 GetJointLocalPosition(ulong UserId, int joint)
	{
        int parent = RemoteKinectWrapper.GetSkeletonJointParent(joint);
	...
	public Quaternion GetJointLocalOrientation(ulong UserId, int joint, bool flip)
	{
        int parent = RemoteKinectWrapper.GetSkeletonJointParent(joint);
	...
	public Vector3 GetDirectionBetweenJoints(ulong UserId, int baseJoint, int nextJoint, bool flipX, bool flipZ)
	{
	...
	public void DetectGesture(ulong UserId, KinectGestures.Gestures gesture)
	{
...
//				break;
	...
	public bool ResetGesture(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
		if(index < 0)
			return false;
		...
		gestureData.complete = false;
		gestureData.cancelled = false;
		gestureData.startTrackingAtTime = Time.realtimeSinceStartup + KinectCommon.Constants.MinTimeBetweenSameGestures;
	...
	public bool DeleteGesture(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
		if(index < 0)
			return false;
	...
	public void ClearGestures(ulong UserId)
	{
		if(UserId == Player1ID)
		{
	...
	public List<KinectGestures.Gestures> GetGesturesList(ulong UserId)
	{
	...
	public bool IsGestureDetected(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
		return index >= 0;
	}
	...
	public bool IsGestureComplete(ulong UserId, KinectGestures.Gestures gesture, bool bResetOnComplete)
	{
		int index = GetGestureIndex(UserId, gesture);
	...
	public bool IsGestureCancelled(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
	...
	public float GetGestureProgress(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
	...
	public Vector3 GetGestureScreenPos(ulong UserId, KinectGestures.Gestures gesture)
	{
		int index = GetGestureIndex(UserId, gesture);
	...
	public void ResetFilters()
	{
		...
		int skeletonJointsCount = (int)KinectCommon.NuiSkeletonPositionIndex.Count;
	...
	void Awake()
	{
		...
		try
		{
			hr = RemoteKinectWrapper.NuiInitialize(KinectCommon.NuiInitializeFlags.UsesSkeleton |
				KinectCommon.NuiInitializeFlags.UsesDepthAndPlayerIndex |
				(ComputeColorMap ? KinectCommon.NuiInitializeFlags.UsesColor : 0));
            if (hr != 0)
			{
            	throw new Exception("NuiInitialize Failed");
			}
			
			hr = RemoteKinectWrapper.NuiSkeletonTrackingEnable(IntPtr.Zero, 8);  // 0, 12,8
			if (hr != 0)
			{
			...
			if(ComputeUserMap)
			{
				hr = RemoteKinectWrapper.NuiImageStreamOpen(KinectCommon.NuiImageType.DepthAndPlayerIndex, 
					KinectCommon.Constants.DepthImageResolution, 0, 2, IntPtr.Zero, ref depthStreamHandle);
				if (hr != 0)
				{
			...
			if(ComputeColorMap)
			{
				hr = RemoteKinectWrapper.NuiImageStreamOpen(KinectCommon.NuiImageType.Color, 
					KinectCommon.Constants.ColorImageResolution, 0, 2, IntPtr.Zero, ref colorStreamHandle);
				if (hr != 0)
				{
			...
			RemoteKinectWrapper.NuiCameraElevationSetAngle(SensorAngle);
			
			// init skeleton structures
			skeletonFrame = new KinectCommon.NuiSkeletonFrame() 
							{ 
								SkeletonData = new KinectCommon.NuiSkeletonData[KinectCommon.Constants.SkeletonCount] 
							};
			
			// values used to pass to smoothing function
			smoothParameters = new KinectCommon.NuiTransformSmoothParameters();
			...
			trackingStateFilter = new TrackingStateFilter[KinectCommon.Constants.SkeletonMaxTracked];
			for(int i = 0; i < trackingStateFilter.Length; i++)
			{
			...
			boneOrientationFilter = new BoneOrientationsFilter[KinectCommon.Constants.SkeletonMaxTracked];
			for(int i = 0; i < boneOrientationFilter.Length; i++)
			{
			...
			clippedLegsFilter = new ClippedLegsFilter[KinectCommon.Constants.SkeletonMaxTracked];
			for(int i = 0; i < clippedLegsFilter.Length; i++)
			{
			...
			gestureTrackingAtTime = new float[KinectCommon.Constants.SkeletonMaxTracked];
		...
		{
			string message = e.Message + " - " + RemoteKinectWrapper.GetNuiErrorString(hr);
			Debug.LogError(message);
			Debug.LogError(e.ToString());
			if(CalibrationText != null)
				CalibrationText.GetComponent<GUIText>().text = message;
		...
		if(ComputeUserMap)
		{
	        ...
	        usersMapSize = RemoteKinectWrapper.GetDepthWidth() * RemoteKinectWrapper.GetDepthHeight();
	        usersLblTex = new Texture2D(RemoteKinectWrapper.GetDepthWidth(), RemoteKinectWrapper.GetDepthHeight());
		...
		if(ComputeColorMap)
		{
	        ...
	        usersClrTex = new Texture2D(RemoteKinectWrapper.GetColorWidth(), RemoteKinectWrapper.GetColorHeight());

			colorImage = new Color32[RemoteKinectWrapper.GetColorWidth() * RemoteKinectWrapper.GetColorHeight()];
	...
	void Update()
	{
		if(KinectInitialized)
		{
			...
			//KinectCommon.UpdateKinectSensor();
			
	        // If the players aren't all calibrated yet, draw the user map.
			if(ComputeUserMap)
			{
				if(depthStreamHandle != IntPtr.Zero &&
					RemoteKinectWrapper.PollDepth(depthStreamHandle, KinectCommon.Constants.IsNearMode, ref usersDepthMap))
				{
		        	UpdateUserMap();
				}
			}
			...
			if(ComputeColorMap)
			{
				if(colorStreamHandle != IntPtr.Zero &&
					RemoteKinectWrapper.PollColor(colorStreamHandle, ref usersColorMap, ref colorImage))
				{
					UpdateColorMap();
				}
			}
			
			if(RemoteKinectWrapper.PollSkeleton(ref smoothParameters, ref skeletonFrame))
			{
				ProcessSkeleton();
			}
					...
					if(gestureData.complete)
					{
						...
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							if(listener.GestureCompleted(Player1ID, 0, gestureData.gesture, 
							                             (KinectCommon.NuiSkeletonPositionIndex)gestureData.joint, gestureData.screenPos))
							{
					...
					else if(gestureData.cancelled)
					{
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							if(listener.GestureCancelled(Player1ID, 0, gestureData.gesture, 
							                             (KinectCommon.NuiSkeletonPositionIndex)gestureData.joint))
							{
	...
	void OnApplicationQuit()
	{
		...
		{
			// Shutdown OpenNI
			RemoteKinectWrapper.NuiShutdown();
			instance = null;
		}
	}
		...
		if(KinectInitialized)
		{
	        if(ComputeUserMap && (/**(allUsers.Count == 0) ||*/ DisplayUserMap))
	        {
				if(usersMapRect.width == 0 || usersMapRect.height == 0)
				{
					...
					{
						DisplayMapsWidthPercent = (RemoteKinectWrapper.GetDepthWidth() / 2) * 100 / cameraRect.width;
					}
					
					float displayMapsWidthPercent = DisplayMapsWidthPercent / 100f;
					float displayMapsHeightPercent = displayMapsWidthPercent * RemoteKinectWrapper.GetDepthHeight() / RemoteKinectWrapper.GetDepthWidth();
			...
			else if(ComputeColorMap && (/**(allUsers.Count == 0) ||*/ DisplayColorMap))
			{
				if(usersClrRect.width == 0 || usersClrTex.height == 0)
				{
					...
					float displayMapsWidthPercent = DisplayMapsWidthPercent / 100f;
					float displayMapsHeightPercent = displayMapsWidthPercent * RemoteKinectWrapper.GetColorHeight() / RemoteKinectWrapper.GetColorWidth();
...
//					}
        ...
        for (int i = 0; i < usersMapSize; i++)
        {
			...
			if(nowUserPixel != wasUserPixel)
			{
	            ...
	            else
	            {
					if(colorImage != null)
					{
						int x = i % KinectCommon.Constants.DepthImageWidth;
						int y = i / KinectCommon.Constants.DepthImageWidth;
	
						int cx, cy;
						int hr = RemoteKinectWrapper.NuiImageGetColorPixelCoordinatesFromDepthPixelAtResolution(
							KinectCommon.Constants.ColorImageResolution,
							KinectCommon.Constants.DepthImageResolution,
							ref pcViewArea,
							x, y, usersDepthMap[i],
						...
						if(hr == 0)
						{
							int colorIndex = cx + cy * KinectCommon.Constants.ColorImageWidth;
    ...
    void CalibrateUser(ulong UserId, int UserIndex, ref KinectCommon.NuiSkeletonData skeletonData)
    {
	...
	private const int stateTracked = (int)KinectCommon.NuiSkeletonPositionTrackingState.Tracked;
	private const int stateNotTracked = (int)KinectCommon.NuiSkeletonPositionTrackingState.NotTracked;
	
	private int [] mustBeTrackedJoints = { 
		(int)KinectCommon.NuiSkeletonPositionIndex.AnkleLeft,
		(int)KinectCommon.NuiSkeletonPositionIndex.FootLeft,
		(int)KinectCommon.NuiSkeletonPositionIndex.AnkleRight,
		(int)KinectCommon.NuiSkeletonPositionIndex.FootRight,
	};
	
	// Process the skeleton data
	void ProcessSkeleton()
	{
        //Debug.Log("Enter ProcessSkeleton");
		...
		for(int i = 0; i < KinectCommon.Constants.SkeletonCount; i++)
		{
			KinectCommon.NuiSkeletonData skeletonData = skeletonFrame.SkeletonData[i];
			ulong userId = skeletonData.TrackingId;
			
			if(skeletonData.eTrackingState == KinectCommon.NuiSkeletonTrackingState.SkeletonTracked)
			{
				...
				if(!AllPlayersCalibrated)
				{
					...
					{
						for(int j = 0; j < KinectCommon.Constants.SkeletonCount; j++)
						{
							if(j != i)
							{
								KinectCommon.NuiSkeletonData skeletonDataOther = skeletonFrame.SkeletonData[j];
								
								if((skeletonDataOther.eTrackingState == KinectCommon.NuiSkeletonTrackingState.SkeletonTracked) &&
				...
				//KinectCommon.NuiSkeletonBoneOrientation[] jointOrients = new KinectCommon.NuiSkeletonBoneOrientation[(int)KinectCommon.NuiSkeletonPositionIndex.Count];
				//KinectCommon.NuiSkeletonCalculateBoneOrientations(ref skeletonData, jointOrients);
				   ...
				   (MaxUserDistance <= 0f || Mathf.Abs(skeletonPos.z) <= MaxUserDistance))
				{
					...
					for (int j = 0; j < (int)KinectCommon.NuiSkeletonPositionIndex.Count; j++)
					{
						bool playerTracked = IgnoreInferredJoints ? (int)skeletonData.SkeletonTrackingState[j] == stateTracked :
							(Array.BinarySearch(mustBeTrackedJoints, j) >= 0 ? (int)skeletonData.SkeletonTrackingState[j] == stateTracked :
							(int)skeletonData.SkeletonTrackingState[j] != stateNotTracked);
...
//						if(j == (int)KinectCommon.NuiSkeletonPositionIndex.SpineBase)
//						{
...
//						}
					...
					RemoteKinectWrapper.GetSkeletonJointOrientation(ref player1JointsPos, ref player1JointsTracked, ref player1JointsOri);
					...
					// get player rotation
					player1Ori = player1JointsOri[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase];
					...
					if(Time.realtimeSinceStartup >= gestureTrackingAtTime[0])
					{
						...
						{
							sDebugGestures += string.Format("\n HandLeft: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.HandLeft] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.HandLeft].ToString() : "");
							sDebugGestures += string.Format("\n HandRight: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.HandRight] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.HandRight].ToString() : "");
							sDebugGestures += string.Format("\n ElbowLeft: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.ElbowLeft] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.ElbowLeft].ToString() : "");
							sDebugGestures += string.Format("\n ElbowRight: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.ElbowRight] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.ElbowRight].ToString() : "");

							sDebugGestures += string.Format("\n ShoulderLeft: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.ShoulderLeft] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.ShoulderLeft].ToString() : "");
							sDebugGestures += string.Format("\n ShoulderRight: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.ShoulderRight] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.ShoulderRight].ToString() : "");
							
							sDebugGestures += string.Format("\n Neck: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.SpineShoulder] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.SpineShoulder].ToString() : "");
							sDebugGestures += string.Format("\n Hips: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase].ToString() : "");
							sDebugGestures += string.Format("\n HipLeft: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.HipLeft] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.HipLeft].ToString() : "");
							sDebugGestures += string.Format("\n HipRight: {0}", player1JointsTracked[(int)KinectCommon.NuiSkeletonPositionIndex.HipRight] ?
							                                player1JointsPos[(int)KinectCommon.NuiSkeletonPositionIndex.HipRight].ToString() : "");
				        ...
				        (MaxUserDistance <= 0f || Mathf.Abs(skeletonPos.z) <= MaxUserDistance))
				{
					...
					// calculate joint orientations
					RemoteKinectWrapper.GetSkeletonJointOrientation(ref player2JointsPos, ref player2JointsTracked, ref player2JointsOri);
					...
					// get player rotation
					player2Ori = player2JointsOri[(int)KinectCommon.NuiSkeletonPositionIndex.SpineBase];
	...
	private void DrawSkeleton(Texture2D aTexture, ref KinectCommon.NuiSkeletonData skeletonData, ref bool[] playerJointsTracked)
	{
		int jointsCount = (int)KinectCommon.NuiSkeletonPositionIndex.Count;
		
		for(int i = 0; i < jointsCount; i++)
		{
			int parent = RemoteKinectWrapper.GetSkeletonJointParent(i);
			
			if(playerJointsTracked[i] && playerJointsTracked[parent])
			{
				Vector3 posParent = RemoteKinectWrapper.MapSkeletonPointToDepthPoint(skeletonData.SkeletonPositions[parent]);
				Vector3 posJoint = RemoteKinectWrapper.MapSkeletonPointToDepthPoint(skeletonData.SkeletonPositions[i]);
				
//				posParent.y = KinectCommon.Constants.ImageHeight - posParent.y - 1;
//				posJoint.y = KinectCommon.Constants.ImageHeight - posJoint.y - 1;
//				posParent.x = KinectCommon.Constants.ImageWidth - posParent.x - 1;
//				posJoint.x = KinectCommon.Constants.ImageWidth - posJoint.x - 1;
	...
	{
		int width = a_Texture.width;  // KinectCommon.Constants.DepthImageWidth;
		int height = a_Texture.height;  // KinectCommon.Constants.DepthImageHeight;
	...
	private int GetGestureIndex(ulong UserId, KinectGestures.Gestures gesture)
	{
		if(UserId == Player1ID)
		{
			int listSize = player1Gestures.Count;
	...
	private bool CheckForCalibrationPose(ulong userId, ref KinectGestures.Gestures calibrationGesture, 
		ref KinectGestures.GestureData gestureData, ref KinectCommon.NuiSkeletonData skeletonData)
	{
		...
		int stateTracked = (int)KinectCommon.NuiSkeletonPositionTrackingState.Tracked;
		int stateNotTracked = (int)KinectCommon.NuiSkeletonPositionTrackingState.NotTracked;
		...
		{
			jointsTracked[j] = Array.BinarySearch(mustBeTrackedJoints, j) >= 0 ? (int)skeletonData.SkeletonTrackingState[j] == stateTracked :
				(int)skeletonData.SkeletonTrackingState[j] != stateNotTracked;
