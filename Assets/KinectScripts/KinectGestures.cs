...
public class KinectGestures
{
	...
	public interface GestureListenerInterface
	{
		...
		void GestureInProgress(ulong userId, int userIndex, Gestures gesture, float progress, 
		...
		bool GestureCompleted(ulong userId, int userIndex, Gestures gesture,
		                      KinectCommon.NuiSkeletonPositionIndex joint, Vector3 screenPos);

		// Invoked if a gesture is cancelled.
		// Returns true, if the gesture detection must be retarted, false otherwise
		bool GestureCancelled(ulong userId, int userIndex, Gestures gesture, 
	...
	private const int leftHandIndex = (int)KinectCommon.NuiSkeletonPositionIndex.HandLeft;
	private const int rightHandIndex = (int)KinectCommon.NuiSkeletonPositionIndex.HandRight;
		
	private const int leftElbowIndex = (int)KinectCommon.NuiSkeletonPositionIndex.ElbowLeft;
	private const int rightElbowIndex = (int)KinectCommon.NuiSkeletonPositionIndex.ElbowRight;
		
	private const int leftShoulderIndex = (int)KinectCommon.NuiSkeletonPositionIndex.ShoulderLeft;
	private const int rightShoulderIndex = (int)KinectCommon.NuiSkeletonPositionIndex.ShoulderRight;
	
	private const int SpineBaseIndex = (int)KinectCommon.NuiSkeletonPositionIndex.SpineBase;
	private const int SpineShoulderIndex = (int)KinectCommon.NuiSkeletonPositionIndex.SpineShoulder;
	private const int leftHipIndex = (int)KinectCommon.NuiSkeletonPositionIndex.HipLeft;
	private const int rightHipIndex = (int)KinectCommon.NuiSkeletonPositionIndex.HipRight;
	...
	private static void SetScreenPos(ulong userId, ref GestureData gestureData, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
...
//			}
			...
			if(jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && 
				jointsTracked[leftShoulderIndex] && jointsTracked[rightShoulderIndex])
			{
				Vector3 neckToHips = jointsPos[SpineShoulderIndex] - jointsPos[SpineBaseIndex];
				...
				if(gestureData.joint == rightHandIndex)
				{
					...
					gestureData.tagVector.y = jointsPos[SpineBaseIndex].y;
				}
				else
				{
...
//			gestureData.screenPos.y = Mathf.Clamp01((gestureData.tagFloat / 2 + shoulderToHand.y) / gestureData.tagFloat);
	...
	private static void SetZoomFactor(ulong userId, ref GestureData gestureData, float initialZoom, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
...
//	private static void SetWheelRotation(ulong userId, ref GestureData gestureData, Vector3 initialPos, Vector3 currentPos)
//	{
...
//	}
	...
	public static void CheckForGesture(ulong userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		if(gestureData.complete)
			return;
		
		float bandSize = (jointsPos[SpineShoulderIndex].y - jointsPos[SpineBaseIndex].y);
		float gestureTop = jointsPos[SpineShoulderIndex].y + bandSize / 2;
		float gestureBottom = jointsPos[SpineShoulderIndex].y - bandSize;
			...
			case Gestures.RaiseRightHand:
				switch(gestureData.state)
				{
					...
					case 1:  // gesture complete
						...
						CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectCommon.Constants.PoseCompleteDuration);
						break;
				}
				break;
...
//						else
						{
							...
							Vector3 jointPos = jointsPos[gestureData.joint];
							CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, KinectCommon.Constants.ClickStayDuration);
...
//						}
						...
						if(jointsTracked[rightHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
...
//								(jointsPos[rightHandIndex].x - gestureData.jointPos.x) < -0.15f;
							...
							bool isInPose = jointsTracked[rightHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
...
//						}
						...
						if(jointsTracked[leftHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
...
//								(jointsPos[leftHandIndex].x - gestureData.jointPos.x) > 0.15f;
							...
							bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
			...
			case Gestures.ZoomOut:
				...
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
				   			jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
				   			jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
							distZoomOut < 0.3f)
						{
					...
					case 1:  // gesture phase 2 = zooming
						if((timestamp - gestureData.timestamp) < 1.5f)
						{
							...
							bool isInPose = jointsTracked[leftHandIndex] && jointsTracked[rightHandIndex] && jointsTracked[SpineBaseIndex] && jointsTracked[SpineShoulderIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
									jointsPos[leftHandIndex].y >= gestureBottom && jointsPos[leftHandIndex].y <= gestureTop &&
									jointsPos[rightHandIndex].y >= gestureBottom && jointsPos[rightHandIndex].y <= gestureTop &&
...
//				                        distWheel, gestureData.tagFloat, Mathf.Abs(distWheel - gestureData.tagFloat)));
			...
			case Gestures.Jump:
				...
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[SpineBaseIndex] && 
							(jointsPos[SpineBaseIndex].y > 0.9f) && (jointsPos[SpineBaseIndex].y < 1.3f))
						{
							SetGestureJoint(ref gestureData, timestamp, SpineBaseIndex, jointsPos[SpineBaseIndex]);
							gestureData.progress = 0.5f;
						}
						break;
					...
					case 1:  // gesture phase 2 = complete
						if((timestamp - gestureData.timestamp) < 1.5f)
						{
							bool isInPose = jointsTracked[SpineBaseIndex] &&
								(jointsPos[SpineBaseIndex].y - gestureData.jointPos.y) > 0.15f && 
								Mathf.Abs(jointsPos[SpineBaseIndex].x - gestureData.jointPos.x) < 0.2f;
			...
			case Gestures.Squat:
				switch(gestureData.state)
				{
					case 0:  // gesture detection - phase 1
						if(jointsTracked[SpineBaseIndex] && 
							(jointsPos[SpineBaseIndex].y <= 0.9f))
						{
							SetGestureJoint(ref gestureData, timestamp, SpineBaseIndex, jointsPos[SpineBaseIndex]);
							gestureData.progress = 0.5f;
						}
